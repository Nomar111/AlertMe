...
local MAJOR, MINOR = "LibCCAlertMe", 1
...
local function CastStart(srcGUID, srcName, srcFlags, castType, spellName, spellID, overrideCastTime, isSrcEnemyPlayer)
...
local function CastStop(srcGUID, srcName, srcFlags, castType, suffix, suffix2)
	local currentCast = casters[srcGUID]
	if currentCast then
		...
		if castType == "CAST" then
			...
			if srcGUID == playerGUID and castingAimedShot then
				...
				callbacks:Fire(event, "player", playerGUID, playerName, srcFlags)
			end
			FireToUnits(event, srcGUID, srcName)
			if suffix2 then
				FireToUnits("UNIT_SPELLCAST_"..suffix2, srcGUID, srcName, srcFlags)
			end
		else
			FireToUnits("UNIT_SPELLCAST_CHANNEL_STOP", srcGUID, srcName, srcFlags)
		end
	end
end
...
function f:COMBAT_LOG_EVENT_UNFILTERED(event)
	...
	if eventType == "SPELL_CAST_START" then
		if isSrcPlayer then
			...
			if isCasting then
				...
				CastStart(srcGUID, srcName, srcFlags, "CAST", spellName, spellID, nil, not isSrcFriendlyPlayer)
			end
		else
			local castUID = makeCastUID(srcGUID, spellName)
			local cachedTime = castTimeCache[castUID]
			spellID = NPCspellNameToID[spellName] -- just for the icon
			if not spellID then
				spellID = 4036 -- Engineering Icon
			end
			if cachedTime then
				CastStart(srcGUID, srcName, srcFlags, "CAST", spellName, spellID, cachedTime*1000)
			else
				castTimeCacheStartTimes[srcGUID..castUID] = GetTime()
				CastStart(srcGUID, srcName, srcFlags, "CAST", spellName, spellID, 1500) -- using default 1.5s cast time for now
			end
		end
	elseif eventType == "SPELL_CAST_FAILED" then

		CastStop(srcGUID, srcName, srcFlags, "CAST", "INTERRUPTED", "STOP")
	...
	elseif eventType == "SPELL_CAST_SUCCESS" then
		if isSrcPlayer then
			...
			elseif classChannelsByCast[spellID] then
				...
				if isChanneling then
					local isSrcFriendlyPlayer = bit_band(srcFlags, COMBATLOG_OBJECT_REACTION_FRIENDLY) > 0
					CastStart(srcGUID, srcName, srcFlags, "CHANNEL", spellName, spellID, nil, not isSrcFriendlyPlayer)
				end
				return
			end
		end
		...
		CastStop(srcGUID, srcName, srcFlags, nil, "SUCCEEDED", "STOP")

	elseif eventType == "SPELL_INTERRUPT" then

		CastStop(dstGUID, dstName, dstFlags, nil, "INTERRUPTED", "STOP")
...
local Passthrough = function(self, event, unit, cast, spellId, ...)--guid, name, cast, spellId)
	-- if unit == "player" or UnitIsUnit(unit, "player") then
	-- 	local spellName, icon, castTime, startTime, endTime
	-- 	if spellId then
	-- 		spellName, _, icon, castTime = GetSpellInfo(spellId)
	-- 		startTime = GetTime() * 1000
	-- 		endTime = startTime + castTime
	-- 	end
	--     --callbacks:Fire(event, unit, playerGUID, playerName, spellName, spellId, icon, startTime, endTime)
	-- end
end
...
FireToUnits = function(event, guid, name, flags, ...)
	if guid == playerGUID then
		local spellName, _, icon, startTime, endTime, _, castID, _, spellId = CastingInfo()
		callbacks:Fire(event, "player", playerGUID, playerName, flags, spellName, spellId, icon, startTime, endTime)
	else
		local commonUnit
		local reportUnit = "noUnit"
		...
		callbacks:Fire(event, reportUnit, guid, name, flags, ...)
	end
end
...
-- 		end
-- 		local partyUnit = partyGUIDtoUnit[guid]
-- 		if partyUnit then return partyUnit end
-- 		local raidUnit = raidGUIDtoUnit[guid]
-- 		if raidUnit then return raidUnit end
-- 		local nameplateUnit = nameplateGUIDtoUnit[guid]
