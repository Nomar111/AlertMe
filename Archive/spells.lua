...
local GetSpellInfo, IsSpellKnown, IsPlayerSpell = GetSpellInfo, IsSpellKnown, IsPlayerSpell
local math, tonumber = math, tonumber
-- set engine as new global environment
setfenv(1, _G.AlertMe)

function S.SafeToNumber(input)
  local nr = tonumber(input)
  return nr and (nr < 2147483648 and nr > -2147483649) and nr or nil
end

function S.IsSpellKnown(spell, pet)
  if (pet) then
...
function S.GetIcon(name)
	...
	local icons = S.cache[name]
	local bestMatch = nil
	if (icons) then
		for spellId, icon in pairs(icons) do
			if (not bestMatch) then
				bestMatch = spellId
			elseif(type(spellId) == "number" and S.IsSpellKnown(spellId)) then
				bestMatch = spellId
			end
		end
	end
...
function S.AddIcon(name, id, icon)
	if name then
		S.cache[name] = S.cache[name] or {}
		if id and icon then
			S.cache[name][id] = icon
		end
	end
end
...
function S.BestKeyMatch(nearkey)
	local bestKey = ""
	local bestDistance = math.huge
	local partialMatches = {}
	if S.cache[nearkey] then
		return nearkey
	end
	for key, value in pairs(S.cache) do
...
function S.CorrectAuraName(input)
  local spellId = S.SafeToNumber(input)
  dprint(1, "spellid", spellId)
  if(spellId) then
    local name, _, icon = GetSpellInfo(spellId)
    if(name) then
      S.AddIcon(name, spellId, icon)
      return name, spellId
    else
      return "Invalid Spell ID"
    end
  else
    local ret = S.BestKeyMatch(input)
    if(ret == "") then
